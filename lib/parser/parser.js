...
var Parser = elf.Object.clone(function () {
  ...
  this.parseUntil = function (value, opts) {
    ...
    if (stmts.length === 1 && stmts[0].value === value) return [];

    while (this.token.value !== value && this.token.value !== "(eof)") {
      var stmt = this.statement();
      if (opts.optional && this.token.value !== value && this.token.value !== opts.step) {
        backtrack.push(stmt);
        return stmts;
      }
